...
## 🦊 Установка в Firefox (Developer Edition / Nightly)

> Поддерживается манифест v3, поэтому рекомендуется использовать актуальный Firefox Developer Edition или Nightly.
...
3. В появившемся диалоге выберите файл `manifest.json` из папки проекта.
4. Перейдите на страницу Django Admin «Добавить новость» и убедитесь, что:
   - появляется зона Drag’n’Drop в блоке «Галерея»;
   - при попытке отправки формы с ошибками поля и файлы восстанавливаются.

Такое подключение временное: после перезапуска Firefox расширение нужно будет загрузить заново через `about:debugging`.
